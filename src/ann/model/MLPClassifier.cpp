...
double_tensor MLPClassifier::predict(
    ...
    bool old_mode = this->m_trainable;
    this->set_working_mode(false);
    size_t sample_count = pLoader->get_sample_count();
    size_t output_dim = this->get_num_classes();

    double_tensor results = xt::empty<double>({sample_count, output_dim});
;
    ...
    for(auto batch: *pLoader){
        double_tensor X_batch = batch.getData();          
        double_tensor Y_batch = this->forward(X_batch);  
        xt::view(results, xt::range(nsamples, nsamples + X_batch.shape()[0]), xt::all()) = Y_batch;
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    meter.reset_metrics();
    for (auto batch : *pLoader) {
        double_tensor X_batch = batch.getData();          // Lấy dữ liệu đầu vào của batch
        double_tensor t_batch = batch.getLabel();         // Lấy nhãn thật của batch

        double_tensor Y_batch = this->forward(X_batch);   // Lan truyền tiến để dự đoán
        ulong_tensor y_true = xt::argmax(t_batch, 1);     // Chuyển nhãn thật thành chỉ số lớp
        ulong_tensor y_pred = xt::argmax(Y_batch, 1);     // Chuyển nhãn dự đoán thành chỉ số lớp

        // Ghi nhận hiệu suất cho từng batch
...
double_tensor MLPClassifier::forward(double_tensor X){
    double_tensor Y = X;
    for (DLinkedList<ILayer*>::Iterator it = m_layers.begin(); it != m_layers.end(); it++) {
        Y = (*it)->forward(Y);
    }
    return Y;
}
