...
Heap<T>::Heap(
        int (*comparator)(T&, T&), 
        void (*deleteUserData)(Heap<T>* ) ) : deleteUserData(deleteUserData), comparator(comparator)
        {
    this->capacity = 10;
    this->count = 0;
...
void Heap<T>::reheapUp(int position){
    ...
    while (position > 0 && compare(elements[position], elements[parent_position]) == -1) {
...
void Heap<T>::reheapDown(int position) {
    ...
    if (left_child < count && compare(elements[left_child], elements[max_index]) == -1) {
        max_index = left_child;
    }
    if (right_child < count && compare(elements[right_child], elements[max_index]) == -1) {
        max_index = right_child;
    }
    if (max_index != position) {
        swap(position, max_index);
        reheapDown(max_index);
    }
}
