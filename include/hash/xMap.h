...
public:
    ...
    public:
        ...
        static bool EntryEqual(Entry*& lhs, Entry*&rhs) {
...
xMap<K,V>::xMap(
                ...
                void (*deleteKeys)(xMap<K,V>* pMap) ) : hashCode(hashCode), loadFactor(loadFactor), valueEqual(valueEqual), deleteValues(deleteValues), 
    ...
    for (int i = 0; i < capacity; i++) {
        this->table[i].setItemEqual(&Entry::EntryEqual);
    }
}
...
xMap<K,V>::xMap(const xMap<K,V>& map) {
    ...
    for (int i = 0; i < this->capacity; i++) {
        ...
        this->table[i].setItemEqual(&Entry::EntryEqual);
        for (auto pEntry : list) {
            Entry* newEntry = new Entry(*pEntry); 
...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    ...
    for (typename DLinkedList<Entry*>::Iterator it = table[index].begin(); it != table[index].end(); it++) {
        if (keyEQ((*it)->key, key)) {
            ...
            count--;
            V retValue = (*it)->value;
            table[index].removeItem(*it,&xMap::deleteEntry);
            return retValue;
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
    ...
    for (typename DLinkedList<Entry*>::Iterator it = table[index].begin(); it != table[index].end(); it++) {
        if (keyEQ((*it)->key, key) && valueEQ((*it)->value,value)) {
            ...
            table[index].removeAt(collision_index);
            count--;
            return true;
        }
        collision_index++;
...
DLinkedList<int> xMap<K,V>::clashes(){
    ...
    for (int i = 0; i < capacity; i++) {
        clashesList.add(max(table[i].size() - 1, 0));
    }
    return clashesList;  
}
...
void xMap<K,V>::copyMapFrom(const xMap<K,V>& map){
    ...
    for(int idx=0; idx < map.capacity; idx++){
        ...
        this->table[idx].setItemEqual(&Entry::EntryEqual);
        for(auto pEntry: list){
